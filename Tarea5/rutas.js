...
function cargarHome(req, res) { //Funciona usando "http://localhost:3000/?token=123" en la url.
    ...
    res.render('home', {
        nombre: 'Christian Pereda'  //ExpresiOn que usamos en home o
    }); //
}

module.exports = function(app) {    //Y ya podemos usar las rtas en otros archivos. ES UNA FUNCION

    app.use('/assets', express.static(path.join(__dirname, 'assets'))); //Para cargar desde el navegador los estilos. MIDDLEWARE.
